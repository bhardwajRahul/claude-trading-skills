...
2. **Floor, never round up -- enforced by a hard post-condition, not just an epsilon.** `contracts = floor(risk_budget / risk_per_contract)`, with a small absolute-epsilon nudge to recover a contract lost to float representation error at an exact boundary; a hard post-condition then walks `contracts` back down, if needed, until `contracts * risk_per_contract <= risk_budget` holds exactly -- this is the actual guarantee that the risk budget is never exceeded, independent of the epsilon. Zero contracts is a legitimate, fail-closed `NO_TRADE` outcome, not an error.
...
- `references/sizing-methodology.md` -- Formulas, the floor algorithm's epsilon rationale, the fail-closed exit-code convention, and worked examples (ES long, B6 short via gate handoff)
