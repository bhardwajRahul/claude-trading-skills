...
スクリプトはレスポンスを厳格に検証します。正当な `data: []` は「該当イベントなし」として扱いますが、レスポンス構造の破損、非有限数、契約外の `market_tier` はエラーとして非zero終了します。取得失敗や不正データを「イベントなし」と誤認しないフェイルクローズド設計です。
...
CLIが非zeroで終了した場合は、イベントが存在しないとは判断しないでください。APIまたはデータ契約の問題を解消して再実行するまで、イベントリスクは未確認として扱います。
