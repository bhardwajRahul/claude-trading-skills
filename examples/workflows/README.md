...
| [`multi-asset-opportunity-daily/replay-run/`](multi-asset-opportunity-daily/replay-run/) | [`multi-asset-opportunity-daily.yaml`](../../workflows/multi-asset-opportunity-daily.yaml) | daily | required-only | native macro-regime synthesis → manual themes → native ideator → sizer → journal |
| [`multi-asset-opportunity-daily/replay-run-full-path/`](multi-asset-opportunity-daily/replay-run-full-path/) | same | daily | **full-path** | + optional news/catalyst brief |
...
`multi-asset-opportunity-daily/replay.yaml` recomputes the native macro-regime
composite, regime classification, and consistency check from fixture component
results, treats themes and the optional news/catalyst brief as human-approved
contracts bound to the replayed upstream artifacts, runs the native
trade-hypothesis-ideator CLI on a SHA-256-bound bundle, sizes actionable cards
with the native position-sizer CLI, and registers IDEA theses through the native
trader-memory-core ingest and link APIs on disposable state. No FINVIZ/FMP fetch
is executed; hypothesis accept/reject is a human gate decision, and rejected or
research-only forex cards never reach sizing or registration. Every thesis stays
