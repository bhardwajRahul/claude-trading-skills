...
2. **Floor, never round up.** `contracts = floor(risk_budget / risk_per_contract)` -- a relative-epsilon nudge recovers a contract lost to float representation error at an exact boundary, but never rounds a genuinely fractional quotient upward. Zero contracts is a legitimate, fail-closed `NO_TRADE` outcome, not an error.
